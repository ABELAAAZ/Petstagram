...
const PostDetail = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                ...
                const responseComments = await sendRequest(`http://localhost:4000/api/comments/${postId}`,'GET', null, { Authorization: 'Bearer ' + auth.token });
    ...
    const commentSubmitHandler = async event => {
        ...
        try {
            ...
            await sendRequest(`http://localhost:4000/api/posts/${postId}/comment`,
                'POST',
                JSON.stringify({
    ...
    return (
        <React.Fragment>
            ...
            <div className='main-content'>
                ...
                <div className='center'>
                    {!isLoading && loadedPosts && <CommentList postId={postId} item={loadedComments} />}
                </div>
            </div>
        </React.Fragment >
    );
