...
const UpdatePost = () => {
    ...
    const postUpdateSubmitHandler = async event => {
        ...
        try {
            await sendRequest(`http://localhost:4000/api/posts/${postId}`, 'PATCH', JSON.stringify({
    ...
    if (!loadedPosts && !error) {
        return (
            <div className='center'>
                <Card><h2>Could not find the post</h2></Card>
    ...
    return (
        <React.Fragment>
            ...
            {!isLoading && loadedPosts && <form className='place-form' onSubmit={postUpdateSubmitHandler} >
                <Input id="title"
                    element="input"
                    type="text"
                    label="Title"
